...
public class Airport {
    ...
    public static void newDay(Airline airLine, String origin, ArrayList<Passenger> paxInAirport) {
        ...
        {//begin outer for loop
            ...
            if(paxInAirport.size() > 0) {//begin if passengers are in airport
                for (int paxElement = 0; paxElement < paxInAirport.size(); paxElement++) {//begin passenger status updater
                    ...
                    if (!(passengerUtility.isAtGate())) {//begin if passenger needs to move

                        passengerUtility.movePassenger();

                    }//end if passenger needs to move

                    if(passengerUtility.getSecurityToGate() == 0 && (!(passengerUtility.isAtGate()))) {//begin if passenger can be added to gate
                    ...
                    if (passengerUtility.isAtGate() && ((min > (paxFlight.getDepartureTime() - 15)) && min < paxFlight.getDepartureTime())) {//begin if passenger can board plane
            ...
            for(Gate gate : gates) {
                if(gate.gs.equals(Gate.gateStatus.EMPTY)) {//begin if statement to cycle planes from deque that have left gates
    ...
    //FILLING OUT ARRAY WITH DESTINATIONS
    ...
    public static void genFlightAndInfo(Airline airLine, String origin, int minutes, Gate gate) {
        Plane plane = new Plane(airLine.getAirlineFleet().get(0).getPaxCapacity());
